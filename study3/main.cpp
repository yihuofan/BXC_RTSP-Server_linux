/*
网络头文件：移除了 <WinSock2.h>, <WS2tcpip.h>, <windows.h>、添加了 <sys/socket.h>, <netinet/in.h>, <arpa/inet.h>, <unistd.h>

Socket初始化：移除了Windows WSA初始化代码

setsockopt参数：将 (const char*)&on 改为 &on

睡眠函数：将 Sleep(1) 改为 usleep(23223)

Socket关闭：将 closesocket() 改为 close()

类型转换：修正了 strlen(sdp) 的类型转换

文件关闭：添加了 fclose(fp) 防止内存泄漏

布尔值：将 true 改为 1

profile长度修改为 2 bit

添加将mp4视频文件中的音频流提取为AAC格式的功能
*/
...
// 在文件开头添加全局变量
static char g_aacFilePath[256] = {0};

// 音频文件准备函数
static int prepareAudioFileComplete(const char *inputFile, char *outputAacFile, size_t outputSize)
{
    ...
    // 生成AAC文件名
    snprintf(outputAacFile, outputSize, "%s_extracted.aac", inputFile);

    // 首先尝试直接拷贝音频流（速度快）
    snprintf(command, sizeof(command),
             "ffmpeg -i \"%s\" -vn -acodec copy -y \"%s\" 2>/dev/null",
             inputFile, outputAacFile);

    ret = system(command);
    if (ret != 0)
    {
        printf("Direct copy failed, trying to re-encode...\n");
        // 如果copy失败，尝试重新编码
        snprintf(command, sizeof(command),
                 "ffmpeg -i \"%s\" -vn -acodec aac -b:a 128k -y \"%s\" 2>/dev/null",
                 inputFile, outputAacFile);

        ret = system(command);
        if (ret != 0)
        {
            printf("Failed to extract audio\n");
            return -1;
        }
    }

    // 检查输出文件是否生成
    if (access(outputAacFile, F_OK) != 0)
    {
        printf("AAC file was not created: %s\n", outputAacFile);
        return -1;
    }

    printf("Audio extraction completed successfully: %s\n", outputAacFile);
    return 1; // 返回1表示是从视频提取的音频
}
...
static int parseAdtsHeader(uint8_t *in, struct AdtsHeader *res) // 解析ADTS头
{
...
static void doClient(int clientSockfd, const char *clientIP, int clientPort)
{
    ...
    while (1)
    {
        ...
        if (!strcmp(method, "PLAY"))
        {
            ...
            // 使用全局变量中的AAC文件路径，如果为空则使用默认路径
            const char *aacFile = strlen(g_aacFilePath) > 0 ? g_aacFilePath : AAC_FILE_NAME;

            FILE *fp = fopen(aacFile, "rb");
            if (!fp)
            {
                printf("读取 %s 失败\n", aacFile);
                break;
...
int main()
{
    ...
    int ret;
    char actualAacFile[256];
    const char *inputFile = "../data/test.mp4";

    ret = prepareAudioFileComplete(inputFile, actualAacFile, sizeof(actualAacFile));
    if (ret < 0)
    {
        printf("Failed to prepare audio file, using default AAC file\n");
        // 如果提取失败，使用默认AAC文件
        strcpy(g_aacFilePath, AAC_FILE_NAME);
    }
    else
    {
        // 设置全局变量
        strcpy(g_aacFilePath, actualAacFile);
        printf("Using extracted audio file: %s\n", actualAacFile);
    }
