/*
头文件：将 Windows Socket 相关的头文件 (WinSock2.h, WS2tcpip.h, windows.h) 替换为 Linux/POSIX 标准的头文件 (sys/socket.h, netinet/in.h, arpa/inet.h, unistd.h)。
Socket 初始化/清理：移除了 Windows 特有的 WSAStartup 和 WSACleanup 函数调用。
Socket API：将 closesocket() 替换为标准的 close()。
修改了 setsockopt() 的参数，移除了 Windows 特有的 (const char*) 类型转换。
延时函数：将 Sleep() (毫秒级) 替换为 usleep() (微秒级)。
*/
